...
function qso(call: string, band = '20m', mode = 'CW', startAtMillis = DURING): Record<string, JSONValue> {
...
function run(
  ...
  const scores = qsos.map((q) => {
    const r = CWTScorer.scoreQso({ scoresheet: sheet, qso: q, operation, ref, isNewDay: false }, ctx)
    sheet = r.scoresheet
    return r.score
  })
  return { sheet, scores }
...
function total(sheet: Parameters<typeof CWTScorer.summarizeScore>[0]['scoresheet']) {
  return CWTScorer.summarizeScore({ scoresheet: sheet, operation, ref: sessionRef, scope: 'operation' }, ctx).cwt.total
}
...
test('the information page shows the total and explains the unique-call multiplier', () => {
  ...
  for (const scope of ['operation', 'day'] as const) {
    const tally = CWTScorer.summarizeScore(
      { scoresheet: sheet, operation, ref: sessionRef, scope }, ctx,
    ).cwt
