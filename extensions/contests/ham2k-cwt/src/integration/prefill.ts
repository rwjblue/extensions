...
import { firstName, guessedQth } from '../exchange.ts'
...
/** Add exchange suggestions without changing CWT setup, saving, or exports. */
export function createPrefill(cache: FileCache, baseActivity: ActivityHook) {
  ...
  const activity = {
    ...
    async loggingControls(args, ctx) {
      const controls = (await baseActivity.loggingControls?.(args, ctx)) ?? []
