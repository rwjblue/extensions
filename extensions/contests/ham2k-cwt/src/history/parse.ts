...
import type { CallHistoryRecord, ParsedCallHistory, ParseIssue } from './types.ts'
...
function contestComment(comment: string): string | undefined {
  ...
  if (
    /^(?:CWT|CWOPS|CWOPEN|CWO|MST|ICWC-MST|SST|K1USN-?SST|NAQP(?:CW|SSB|RTTY)?|NS(?:LADDER|RTTY)?|ARRL[A-Z\d-]*|CQ[A-Z\d-]*|[A-Z]{2}QP|QSOPARTY(?: [A-Z]{2})?)$/.test(
      tag,
    )
  ) {
    return tag
